...
| [capture.ts](capture.ts) | Playwright script (not yet run). Produces deterministic screenshots across 3 trust levels × 2 viewports × 27 routes. Requires Playwright install + Clerk storage-state setup. See top-of-file docstring. |
...
| [screenshots/](screenshots/) | Output directory for capture.ts. Empty at Phase A — see "Phase A limitations" below. |
...
1. **Screenshots not captured.** [capture.ts](capture.ts) is ready but requires:
