-- ============================================================
-- Migration 014: Alpha-C Trust Model
-- Session: CC-C1a
-- Date: 2026-04-15
-- Idempotent: safe to re-run
-- ============================================================
--
-- Extends the schema for the Alpha-C trust model:
--   - New years_known_bucket enum values (3to5, 5to10, 10plus)
--   - vouches: vouch_score, is_post_stay, source_booking_id, is_staked, updated_at
--   - users: vouch_count_given, vouch_count_received, vouch_power default
--   - listings: visibility_mode, preview_photos, preview_description, access_settings
--   - invites: invitee_name, pre_vouch_data, status
--   - Replaces RPCs and triggers for new formula
-- ============================================================

-- ============================================================
-- 1. EXTEND years_known_bucket ENUM
-- ============================================================
-- Add new Alpha-C bucket values. Old values (lt1yr, 4to7yr, etc.) stay
-- for backward compat but won't be used by new code.
-- PostgreSQL 12+ allows ADD VALUE inside transactions.
...
-- Change vouch_power default from NULL to 1.0 (benefit of the doubt)
ALTER TABLE users ALTER COLUMN vouch_power SET DEFAULT 1.0;

-- Backfill: any existing NULL vouch_power → 1.0
...
-- visibility_mode: replaces the old preview_visibility/full_visibility system
ALTER TABLE listings ADD COLUMN IF NOT EXISTS visibility_mode TEXT NOT NULL DEFAULT 'preview_gated'
...
-- access_settings: canonical visibility control (supersedes min_trust_gate)
ALTER TABLE listings ADD COLUMN IF NOT EXISTS access_settings JSONB NOT NULL DEFAULT '{
...
ALTER TABLE invites ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'sent'
  CHECK (status IN ('sent', 'clicked', 'joined'));

-- ============================================================
-- 6. VOUCH SCORE COMPUTATION TRIGGER
-- ============================================================
-- On vouch insert/update, compute vouch_score from type + years_known_bucket
-- and update vouch counts on the users table.
...
BEGIN
  ...
  -- Years-known multiplier (supports both old and new enum values)
  v_mult := CASE NEW.years_known_bucket
    WHEN 'lt1'     THEN 0.6
    WHEN 'lt1yr'   THEN 0.6
    WHEN '1to3'    THEN 1.0
    WHEN '1to3yr'  THEN 1.0  -- old value, same multiplier in new model
    WHEN '3to5'    THEN 1.2
    WHEN '4to7yr'  THEN 1.2  -- old value, map to nearest new bracket
    WHEN '5to10'   THEN 1.5
    WHEN '8to15yr' THEN 1.5  -- old value, map to nearest new bracket
    WHEN '10plus'  THEN 1.8
    WHEN '15plusyr' THEN 1.8  -- old value, same multiplier
    ELSE 1.0
  END;
...
-- ============================================================
-- New model: vouch_power = avg(guest_rating of vouchees) / 4.0, clamped [0.5, 1.5]
-- Fires when a user's guest_rating changes → updates vouch_power for all their vouchers.

CREATE OR REPLACE FUNCTION recalculate_vouch_power()
RETURNS TRIGGER AS $$
BEGIN
  -- Update vouch_power for every user who vouched for this user
  UPDATE users
  ...
  -- Also update guest_rating_avg (alias: guest_rating is the avg)
  -- The guest_rating column IS the average — no separate _avg column needed.
...
-- ============================================================
-- On stay_confirmation review → update user rating averages

CREATE OR REPLACE FUNCTION update_user_ratings_on_review()
RETURNS TRIGGER AS $$
BEGIN
  -- Guest rating changed → update guest's guest_rating average
  IF NEW.guest_rating IS NOT NULL AND (OLD.guest_rating IS NULL OR NEW.guest_rating != OLD.guest_rating) THEN
    UPDATE users SET
  ...
  -- Host rating changed → update host's host_rating average
  IF NEW.host_rating IS NOT NULL AND (OLD.host_rating IS NULL OR NEW.host_rating != OLD.host_rating) THEN
    UPDATE users SET
  ...
  -- Listing rating changed → update listing's avg_listing_rating
  IF NEW.listing_rating IS NOT NULL AND (OLD.listing_rating IS NULL OR NEW.listing_rating != OLD.listing_rating) THEN
    UPDATE listings SET
...
-- Also fire on INSERT (in case ratings are set at creation)
...
-- ============================================================
-- 9. RPC: get_trust_data_for_viewer (replaces calculate_one_degree_scores)
-- ============================================================
-- Single query returning all vouch data needed for 1° score computation.
-- Returns one row per mutual-connector path (viewer → connector → target).
...
-- Keep old RPC for backward compat with trust-data.ts
-- (will be removed in CC-C3 when browse/detail pages switch to new lib)
-- calculate_one_degree_scores stays as-is

-- ============================================================
-- 10. RPC: recalculate_vouch_power (single user, on-demand)
-- ============================================================
...
BEGIN
  ...
  IF v_avg_rating IS NULL THEN
    v_result := 1.0;  -- No data = benefit of doubt
  ELSE
...
-- ============================================================
-- 11. RPC: get_user_network (vouched-for, vouched-by, pending invites)
-- ============================================================
